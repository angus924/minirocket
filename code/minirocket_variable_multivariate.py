...
# ** This is an experimental extension of MiniRocket for multivariate, variable-length input.
...
@njit("float32[:,:](float32[:,:,:],int32[:],Tuple((int32[:],int32[:],int32[:],int32[:],float32[:])))", parallel = True, fastmath = True, cache = True)
            ...
            for kernel_index in range(num_kernels):
                ...
                if _padding1 == 0:
                    for feature_count in range(num_features_this_dilation):
                        features[example_index, feature_index_start + feature_count] = _PPV(C, biases[feature_index_start + feature_count]).mean() if len(C) > 0 else 0
                else:
                    for feature_count in range(num_features_this_dilation):
                        features[example_index, feature_index_start + feature_count] = _PPV(C[padding:-padding], biases[feature_index_start + feature_count]).mean() if len(C[padding:-padding]) > 0 else 0
